	...
	def add_object(self, name, obj, index=None):
		...
		if name not in self._objects:
			...
			if not index or index >= len(self._index):
				...
				for i in range(index, len(self._index)-1):
					self._index[i+1] = self._index[i]
				self._index[index] = name
			# Add all containing objects
			if isinstance(obj, Frame):
				for j in range(len(obj._index)):
					n = obj._index[j]
	...
	def rem_object(self, name):
		...
		if name in self._objects:
			...
			if isinstance(self._objects[name], Frame):
				for _,n in self._objects[name]._index.items():
					self.rem_object(n)
			# Update parent
			...
			# Update indexes
			x = dict([(v, k) for (k, v) in self._index.items()])[name]
			for i in range(x+1, len(self._index)-1):
...
class Input(FormObject):
	...
	def __init__(self, label, value='', **kwargs):
		...
		FormObject.__init__(self, value)
		# Separate kwargs
		label_kwargs = dict([(k,v) for k,v in kwargs.items() if k.startswith('label_')])
		input_kwargs = dict([(k,v) for k,v in kwargs.items() if k.startswith('input_')])
		# Create objects
